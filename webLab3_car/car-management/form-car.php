...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   try {
      ...
      if (!empty($_FILES['thumbnail']['tmp_name'])) { # Trường hợp là có
         $pathUpload = "C:\\xampp\\htdocs\\weblab3.php\\weblab3.php\\webLab3_car\\uploads\\";
      ...
      if ($option === "add") {
         $query = "INSERT INTO car (id ,name, brand, color, price, yearOfProduction, description, thumbnail,seats,fuel_type) VALUES (:id ,:name, :brand, :color, :price, :yearOfProduction, :description, :thumbnail,:seats,:fuel_type)";
      } else {
         $query = "UPDATE car SET name=:name, brand=:brand, color=:color, price=:price, seats=:seats,fuel_type=:fuel_type, yearOfProduction=:yearOfProduction, description=:description" . (!empty($_FILES['thumbnail']['tmp_name']) ? ", thumbnail=:thumbnail" : "") . " WHERE id=:id";
      }
      $params = $_POST;
      ...
      if ($result) {
         ...
         if (!empty($_FILES['thumbnail']['tmp_name'])) 
         {
          ...
          # Điều hướng về trang list-car.php sau khi thành công
         header("Location: http://localhost:3000/weblab3.php/webLab3_car/car-management/list-car.php");
         exit();  # Dừng script tại đây sau khi điều hướng
      }
       else{
            $message = "Lỗi dữ liệu!";
            $status = 'danger';
...
<div class="container">
   ...
   <form action="./form-car.php<?= $option === "update" ? "?id=" . $data['0']['id'] : '' ?>" method="post" enctype="multipart/form-data">
      ...
      <div class="row">
         ...
         <div class="form-floating mb-3 col-6">
            <input type="number" min="1" class="form-control" id="seats" required name="seats" placeholder="Số ghế" value="<?= $option === "update" ? $data['0']['seats'] : "" ?>">
            <label class="ms-2" for="seats">Số ghế</label>
         </div>
         <div class="form-floating mb-3 col-6">
            <input type="text" class="form-control" id="fuel_type" required name="fuel_type" placeholder="Loại nhiên liệu" value="<?= $option === "update" ? $data['0']['fuel_type'] : "" ?>">
            <label class="ms-2" for="fuel_type">Loại nhiên liệu</label>
         </div>
      </div>
      <div class="form-floating mb-3">
